@php
    $primaryInstitutionId = (int) old('institution_id', $program->firstBatch?->institution_id ?? 0);
    $themePresets = [
    ...
    <div class="row g-4">
        <div class="col-lg-7">
            <div class="row g-3">
                ...
                <div class="col-md-6"><label class="form-label" for="institution_id">Lembaga penyelenggara</label><select class="form-select @error('institution_id') is-invalid @enderror" id="institution_id" name="institution_id"><option value="">Pilih lembaga</option>@foreach($institutions as $institution)<option value="{{ $institution->id }}" @selected($primaryInstitutionId === $institution->id)>{{ $institution->name }}</option>@endforeach</select>@error('institution_id')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
